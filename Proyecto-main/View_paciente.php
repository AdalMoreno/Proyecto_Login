<?php
    ...
    try {
        ...
        // Obtener datos del paciente, incluyendo la foto
        $sql_paciente = "SELECT nombre, foto FROM Usuario WHERE id_usuario = :id_paciente";
        $stmt_paciente = $conexion->prepare($sql_paciente);
        $stmt_paciente->bindParam(':id_paciente', $id_paciente, PDO::PARAM_INT);
        $stmt_paciente->execute();
        $paciente = $stmt_paciente->fetch(PDO::FETCH_ASSOC);

        // Mostrar la foto en el HTML (si no hay foto, muestra una por defecto)
        $fotoPaciente = !empty($paciente['foto']) ? 'fotos_pacientes/' . htmlspecialchars($paciente['foto']) : 'https://via.placeholder.com/200';
        
        // En guardar_foto.php, antes de mover el nuevo archivo:
        $sql_current = "SELECT foto FROM Usuario WHERE id_usuario = :id_paciente";
        $stmt_current = $conexion->prepare($sql_current);
        $stmt_current->bindParam(':id_paciente', $id_paciente, PDO::PARAM_INT);
...
<body class="font-['Roboto'] bg-gray-100">
        ...
        <div class="min-h-screen flex flex-col md:flex-row">
            ...
            <div class="bg-blue-700 text-white w-full md:w-64 p-4">
                 <div class="flex flex-col items-center mb-8 "> <!-- Añadido mt-8 aquí -->
                <!-- Foto del paciente con funcionalidad editable -->
                <div class="relative">
                   <img id="patientPhoto" src="<?php echo $fotoPaciente; ?>" alt="Foto del paciente" class="w-32 h-32 rounded-full object-cover cursor-pointer hover:opacity-80 transition">
...
<script>
    ...
    function cancelarCita(id_cita) {
        ...
        fetch('eliminar_cita_paciente.php', {
            method: 'POST',
            headers: {
        ...
        .catch(error => {
            console.error('Error:', error);
            showNotification(error.message || "Ocurrió un error al procesar tu solicitud. Por favor, intenta nuevamente.", 'error');
        });
    }
    ...
    document.getElementById('photoUpload').addEventListener('change', function(e) {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.size > 5 * 1024 * 1024) {
                showNotification("La imagen no debe superar los 5MB de tamaño.", 'error');
                return;
            }
